package com.javagda25.jsp_invoicemanager.database;
...
public class InvoiceDao {
    ...
    public List<Invoice> getAllUnpaid() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            criteriaQuery.select(root).where(cb.equal(root.get("paid"), 0));
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            criteriaQuery.select(cb.sum(
                    cb.sum(cb.prod(join.get("price"), join.get("stock")),
                            cb.prod(cb.prod(join.get("price"), join.get("stock")), join.get("taxValue")))))
